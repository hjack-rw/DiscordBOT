...
async def change_leaderboard(interaction: Interaction, member:Member, username:Optional[str], offset:Optional[bool]):
    ...
    is_archived = ExperienceInfo.get_joined_table(get_kwargs={"multiple":False, "only_archived":True}, user_id=member.id, specified_columns=["archived"])
    if is_archived is not False:
        error = f"User {member.nick or member.global_name} "
        if is_archived is True:
            raise Exception(error + "was ARCHIVED")
        else:
            raise Exception(error + "doesn't have a leaderboard card")
...
async def questionnaire_leaderboard(interaction:Interaction, question_idx:Optional[int]):
    ...
    is_archived = ExperienceInfo.get_joined_table(get_kwargs={"multiple":False, "only_archived":True}, user_id=member.id)
    if is_archived is True:
