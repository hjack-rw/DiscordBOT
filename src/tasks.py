...
async def club_events_reminder(server):
    ...
    if (test_bot["test_tasks"] or today.weekday() in [4, 5, 6]):
        ...
        if not test_bot["test_tasks"]:
            ...
            [message.delete() async for message in channel.history(limit=None) if message.author == "Prof. Snape" and message.content == "Mention: <@&1314983531050569828>"]
...
async def print_notification(server, date, event_name, variables=[], is_task=True, same_day=False):
    ...
    if is_task:
        if test_bot["test_tasks"] and event_name not in ["Welcome", "Card - Book of Monsters", "Card - Cornish Pixies", "Club Points"]:
            print(f'''"{events[event_name]}" task running... {datetime.now()}!''')
    else:
        date = date.replace(hour  =time_trigger[event_name].hour,
                            minute=time_trigger[event_name].minute,
                            second=time_trigger[event_name].second,
                            tz    =time_trigger[event_name].tzinfo,)
    ...
    elif events[event_name] == "weekly_cards":
