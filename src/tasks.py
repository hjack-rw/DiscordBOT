...
async def morning_reminder(server):
    ...
    for idx, birthday in enumerate(birthdays, 1):

        if idx == 1:
            description=f"I see something in the stars... Today is a very special day!\nPlease, wish <@{birthday}> a **Happy Birthday** <:hugs:1256225688403447888> :heart:"
        else:
            description=f"Wait! There's more...\nPlease, wish <@{birthday}> a **Happy Birthday** as well <:hugs:1256225688403447888> :heart:" 

        # create birthday notification message
        embed = Embed(color=system_embed_color, description=description)
