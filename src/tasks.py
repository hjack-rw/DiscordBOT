...
async def game_midnight_reminder(server):
    ...
    if (test_bot["test_tasks"] or delta.days % 14 == 0):
        ...
        await set_event_and_notification(server, event_info, end_time=(3,0,0), start_time=(0,0,0), today=today+timedelta(days=1))
...
async def set_event_and_notification(server, event_info, end_time, start_time, today):
    ...
    if test_bot["test_tasks"]:
        ...
        delete_after["hours"], delete_after["minutes"] = end_time[0], start_time[1] - today.minute
        ...
        beginning = today.replace(hour=start_time[0], minute=start_time[1], second=start_time[2])
        ending = beginning + timedelta(hours=end_time[0], minutes=end_time[1], seconds=end_time[2])
        duration = f"~{end_time[0]} hour{'s' if end_time[0] > 1 else ''}"
