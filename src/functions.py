...
async def set_event_and_notification(server, event_info, date, event_duration, start_time, only_hour=True, time_delta=0, for_subscriber=True):
    global delete_after
    ...
    message = await send_webhook(target_channel=channel, user_name=event_info["account"], content=f"Mention: {'<@&1278844289694171260>' if for_subscriber else '<@&1314983531050569828>'}", embed=embed)
...
async def print_notification(server, event_name, date=None, variables=[], is_task=True, same_day=False):
    ...
    if event_name == "Welcome":
        ...
        return await set_event_and_notification(server, event_info, date, event_duration=(1,0,0), start_time=(19,30,0), for_subscriber=False)
