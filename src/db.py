        ...
        if is_new:
            ...
            if self.get_joined_table(**{column_id:kwargs[column_id]}) is None:
                ...
                try:
                    ...
                    await self.joined_table().add(**needed_kwargs)
                except Exception as error:
...
class Database():
    ...
    async def backup(cls):
        ...
        try:
            async with await cls.connect() as con:
                
                # access underlying sqlite3 connection
                raw_conn = con._conn

                with io.open(DUMP_PATH, mode="w", encoding="utf-8") as file:
                    
                    # iterdump() function
                    for line in raw_conn.iterdump():
                        file.write('%s\n' % line)
        
        except aiosqlite.Error as error:
            raise Exception(f"{module_name} BACKUP error: {str(error)}")
    ...
    async def restore(cls):
        ...
        try:
            ...
            raise Exception(f"{module_name} RESTORE error: {str(error)}")
