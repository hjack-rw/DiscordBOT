...
class Database():
    ...
    def restore(cls):
        ...
        SQL3_PATH = os.path.join(cls.database_path, "sqlite3")
        DB_PATH   = os.path.join(cls.database_path, cls.database_name)
        ...
        try:
            ...
            with open(DUMP_PATH, 'r') as dump_file:
                subprocess.run([SQL3_PATH, DB_PATH], stdin=dump_file,
                               check=True, capture_output=True, text=True)

        except subprocess.CalledProcessError as error:
            raise Exception(f"sqlite3 RESTORE error: {error.stderr.strip() or str(error)}")
